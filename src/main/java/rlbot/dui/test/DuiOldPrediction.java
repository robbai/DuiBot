...
public class DuiOldPrediction {
	
	private final static int fps = 15;
	private final static int maxSeconds = 5;
	...
	public static void update(BallData ball, Renderer r){
		...
		for(int i = 1; i < maxSeconds * fps; i++){
			...
			if(render) r.drawLine3d(Color.green, positions[i - 1].toFramework(), positions[i].toFramework());
