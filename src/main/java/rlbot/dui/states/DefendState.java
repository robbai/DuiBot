...
public class DefendState extends State {
	...
	public double getOutput(DuiData d){
		if(!CentraliseState.isBackboardRolling(d.input.ball) && !DuiPrediction.isDanger() && Math.abs(d.carPosition.y) > Math.abs(d.ballPosition.y) && (d.carPosition.distance(Dui.ownGoal) < 2500) || (d.car.team == 0 ? d.carPosition.y < -4800 : d.carPosition.y > 4800)){
...
//			}else 
			if(Math.abs(d.carPosition.y) > 5120){
				Vector2 defence = new Vector2(Math.max(-800, Math.min(800, d.ballPosition.x)), d.car.team == 0 ? -5010 + (d.ballDistance / 50): 5010 - (d.ballDistance / 50));
